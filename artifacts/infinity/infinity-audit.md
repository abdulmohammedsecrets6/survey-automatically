...
- **Interruption**: You can't speak over Infinity AI. The `handleStopSpeaking` kills TTS but the state machine goes back to idle/wake, not recording.
...
**Effect**: ChatGPT muscle memory is to swipe to edit or tap "regenerate". Without this, every mistake costs the entire conversation context. Makes Infinity AI feel like a toy.
...
AWAITING INPUT...
```
No suggested first actions, no tour, no "try saying X" cards, no showcase of capabilities. Users don't know what Infinity AI CAN do.
...
**`home.tsx`** (line 446-469): When speaking to Infinity AI, the transcript text is set optimistically. But the conversation feed is hidden in voice mode — only the orb subtitle shows the latest exchange.
...
**`conversation-feed.tsx`**: Messages show "YOU" / "INFINITY AI" labels but no timestamps. In long conversations, context is lost.
...
**Effect**: User says "hey Infinity AI" and nothing happens. They have to say it again. Feels broken.
...
**Effect**: Users setting up Infinity AI with Llama or Mistral models silently don't get web search with no explanation.
...
If you want Infinity AI to *feel* like a real product, fix these in order:
...
Everything else (#6-#36) is polish. Fix these three and Infinity AI transforms from "interesting prototype" to "I could actually use this every day."
