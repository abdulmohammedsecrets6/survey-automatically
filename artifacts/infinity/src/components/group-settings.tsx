            ...
            <div className="space-y-5 p-5">
              {!group ? (
                ...
                <>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/70">Infinity AI participation</label>
                    <div className="grid grid-cols-2 gap-2">
                      {(['always', 'mention'] as const).map((value) => <button type="button" key={value} onClick={() => setAiToggle(value)} className={`rounded-xl border px-3 py-2.5 text-left text-xs transition ${aiToggle === value ? 'border-primary/50 bg-primary/10 text-primary' : 'border-border/50 hover:bg-secondary/70'}`}>{value === 'always' ? 'Always responds' : 'Only when @Infinity AI'}</button>)}
                    </div>
                  </div>
