...
interface GemDialogProps {
  open: boolean;
  onClose: () => void;
  /** Ask Infinity AI to analyze the loaded data, payload is a text summary. */
...
export function DataLab({ open, onClose, onAskInfinity }: GemDialogProps) {
  const { t } = useI18n();
  ...
  /** Build a compact summary to hand to Infinity AI for analysis. */
  const askInfinity = () => {
    if (!data) return;
    haptics.medium?.();
