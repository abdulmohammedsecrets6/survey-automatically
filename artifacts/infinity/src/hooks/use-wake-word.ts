...
export function useWakeWord({ onWake, onCommand, onError, onCommandTimeout, lang = 'en-US', autoDetectLang = false, onLangChange }: UseWakeWordOptions) {
  ...
  // Without this, "hey Infinity AI" stops working after the user backgrounds the app on iOS/Android.
  useEffect(() => {
   ...
   * restarting it. Call this when Infinity AI starts thinking or speaking.
   */
  ...
  const unsuppress = useCallback(() => {
    ...
    // 800ms cooldown so TTS-echo audio (Infinity AI saying "Infinity AI" in a response,
  ...
  const activateCommand = useCallback((fromGesture = false) => {
    ...
    if (!activeRef.current) {
      if (fromGesture) {
        ...
        // user can tap or say "hey Infinity AI" to continue.
        commandModeRef.current = false;
        onCommandTimeoutRef.current?.();
      }
    }
