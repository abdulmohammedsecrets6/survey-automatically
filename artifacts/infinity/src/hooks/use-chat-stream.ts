...
export function useChatStream(deps: ChatStreamDeps): ChatStreamResult {
  ...
  const processUserText = useCallback(async (userText: string, file?: AttachedFile | null, speak = true, codeAllowance?: boolean, researchMode = false, buildAllowance?: boolean, keyId?: string) => {
    ...
    // If the message looks like a question about Infinity AI's own code and the
    ...
    // it (false), the message is never dropped, Infinity AI still answers.
      ...
      if (!res.ok) {
        try {
          ...
          const hint = res.status >= 500
            ? 'The Infinity AI server is unreachable right now (likely restarting or down). Wait a few seconds and retry.'
          ...
          try {
            ...
            switch (parsed.type) {
              ...
              case 'done':
                ...
                // Auto-follow-up: when Infinity AI signals the next step (Infinity Build
