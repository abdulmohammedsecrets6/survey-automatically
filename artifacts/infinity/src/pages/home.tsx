...
export default function Home() {
  ...
  const [mode, setMode] = useState<'voice' | 'chat' | 'agent' | 'camera'>(() => {
    try {
      const saved = localStorage.getItem('infinity-mode') as any;
  ...
  // Thinking mode, Infinity AI streams a private reasoning pass before the answer
  // (shown in a collapsible "Thinking" block). Persisted across reloads.
  const [thinkingEnabled, setThinkingEnabled] = useState<boolean>(() => {
    try { return localStorage.getItem('infinity-thinking') === 'true'; } catch { return false; }
  });
  useEffect(() => {
    try { localStorage.setItem('infinity-thinking', thinkingEnabled ? 'true' : 'false'); } catch { /* noop */ }
  }, [thinkingEnabled]);
  ...
  const [subtitle, setSubtitle] = useState<{ user: string; infinity: string } | null>(null);
  ...
  useEffect(() => {
    try { micGrantedBeforeRef.current = localStorage.getItem('infinity-mic-granted'); } catch { /* noop */ }
  }, []);
  const markMicIntent = useCallback(() => {
    micIntentRef.current = true;
    try { localStorage.setItem('infinity-mic-intent', 'true'); } catch { /* noop */ }
  }, []);
  ...
  useEffect(() => {
    try { localStorage.setItem('infinity-mode', mode); } catch { /* noop */ }
  }, [mode]);
  ...
  // Activation method: 'wake' for "Hey Infinity AI", 'clap' for double clap
  const [activationMethod, setActivationMethod] = useState<'wake' | 'clap'>(() => {
    try { return (localStorage.getItem('infinity-activation-method') as 'wake' | 'clap') || 'wake'; }
    catch { return 'wake'; }
  });
  ...
  const { start: startWakeWord, stop: stopWakeWord, reset: resetWakeWord, suppress: suppressWakeWord, unsuppress: unsuppressWakeWord, activateCommand } = useWakeWord({
    ...
    onWake: () => {
      ...
      try { localStorage.setItem('infinity-mic-granted', 'true'); } catch { /* noop */ }
      playWakeSound();
      vibrate([50, 30, 50]);
      setStatus('recording');
    },
    ...
    onError: (msg) => {
      ...
      if (msg.includes('denied')) {
        try { localStorage.removeItem('infinity-mic-granted'); } catch { /* noop */ }
  ...
  useEffect(() => {
    try { localStorage.setItem('infinity-activation-method', activationMethod); } catch { /* noop */ }
  }, [activationMethod]);
  const synthesizeSpeech = useSynthesizeSpeech();

  // Free client-side emotion detection (Web Audio prosody) while recording.
  // The detected label is sent to the chat endpoint so Infinity AI can adapt tone.
  ...
  useEffect(() => {
    ...
    const lastInfinity = [...messages].reverse().find(m => m.role === 'assistant');
    if (lastUser || lastInfinity) {
      setSubtitle({ user: lastUser?.content ?? '', infinity: lastInfinity?.content ?? '' });
    }
  }, [messages]);
  ...
  const handleError = useCallback((msg: string, detail?: ErrorDetail, onRetry?: () => void, code?: string) => {
    ...
    if (code === 'llm_cooling') {
      toast({
        variant: 'default',
        title: 'Infinity AI is recharging',
        description: (
  ...
  // ── Infinity Books: poll background jobs + notify when a book is ready ──
  useEffect(() => {
    let cancelled = false;
  ...
  const loadConversation = useCallback(async (id: string) => {
    try {
      const res = await fetch(`/api/infinity/conversations/${id}`);
      if (!res.ok) return;
  ...
  const handleProjectNewChat = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/infinity/conversations', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  // Wave 2, Data Lab hands a data summary to Infinity AI in chat mode
  ...
  const playTTS = useCallback((infinityText: string, onStart: () => void, onDone: () => void) => {
    synthesizeSpeech.mutate(
      { data: { text: infinityText } },
      {
  ...
  const handleChatSubmit = () => {
    ...
    // "@Build <message>" shortcut → auto-switch to Infinity Build with the task.
  ...
  const handleGenerateImage = useCallback(async (prompt: string) => {
    ...
    try {
      const res = await fetch('/api/infinity/generate-image', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleToggleScreenShare = useCallback(async () => {
    if (screenShareActive) {
      ...
      try {
        ...
        toast({ title: 'Screen sharing started', description: 'Infinity AI can now see your screen' });
      } catch {
        setScreenShareActive(false);
  ...
  const refreshBuildFiles = useCallback(async () => {
    try {
      const res = await fetch('/api/infinity/workspace');
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    window.addEventListener('infinity-plugin-action', onPluginAction);
    return () => window.removeEventListener('infinity-plugin-action', onPluginAction);
  }, [handlePluginAction]);
  ...
  // Barge-in recognizer, orb tap while Infinity AI is speaking in chat mode:
  ...
  return (
    <div className={`${resolved} h-dvh bg-background text-foreground flex flex-col overflow-hidden`}>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        {/* ── PiP Floating Window: Infinity AI agent browser ── */}
        <PipBrowserWindow
          open={pipBrowserOpen}
      ...
      <AppOverlays
        ...
        onCancelResearch={async (jobId) => { try { await fetch(`/api/infinity/research/${jobId}/cancel`, { method: 'POST' }); refreshSidebar(); } catch { /* noop */ } }}
        ...
        onCancelBook={async (jobId) => { try { await fetch(`/api/infinity/book/jobs/${jobId}/cancel`, { method: 'POST' }); } catch { /* noop */ } }}
