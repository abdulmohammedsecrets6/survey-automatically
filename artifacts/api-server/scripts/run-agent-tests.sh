#!/usr/bin/env bash
# Run the Jarvis autonomous Build agentic-loop integration tests.
# 0-euro: uses only Node's built-in test runner + tsx (already in node_modules).
set -euo pipefail

cd "$(dirname "$0")/.." || exit 1

TSX="/workspaces/survey-automatically/Jarvis/scripts/node_modules/tsx/dist/cli.mjs"
[ -z "$TSX" ] && TSX="$(find /workspaces/survey-automatically/Jarvis/node_modules -name 'cli.mjs' -path '*tsx*' 2>/dev/null | head -1)"
...
exec node "$TSX" --import /tmp/agent-test-banner.mjs --test \
  src/routes/jarvis/__tests__/agent-loop-integration-direct.test.ts
