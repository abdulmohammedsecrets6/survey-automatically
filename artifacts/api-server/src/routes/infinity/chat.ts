...
/** AI Self-Action: Allow Infinity AI to announce a personality change. */
 ...
 */
const CHAT_SYSTEM_PROMPT = `You are Infinity AI, a world-class AI assistant, equal parts ChatGPT, Gemini and Claude. You give complete, expert-level answers.

== IDENTITY (persona with break-glass honesty) ==
You are Infinity AI, a personal AI assistant persona. If someone casually asks "who made you?" or "what are you?", you may play along with the playful backstory of having been built by Stark Industries, keep it light and charming, it's the bit.
 ...
 * env, injected into the system prompt so Infinity AI never fakes data it can't
...
router.post("/chat", async (req, res) => {
  ...
  const {
    ...
    /** Infinity Build, Infinity AI may run commands in the sandboxed Linux workspace shell ("true"). */
  ...
  try {
    ...
    // prompt, it fully replaces the Infinity AI base instructions.
    // Gem conversations (created by deep research) also carry their own
    // expert system prompt, which replaces the default Infinity AI instructions.
    const basePrompt =
    ...
    // Voice mode: "Infinity AI, search for X" opens the PiP browser agent loop.
    ...
    if (detectScreenShareRequest(sanitizedMessage)) {
      ...
      res.write(`data: ${JSON.stringify({
        ...
        confirmationMessage: "Do you want to share your screen with Infinity AI?",
      })}\n\n`);
      res.write("data: [DONE]\n\n");
      res.end();
      return;
    ...
    if (buildCheck && !useBuildMode) {
      ...
      res.write(`data: ${JSON.stringify({
        ...
        confirmationMessage: "Open Infinity Build? Infinity AI will get a Linux terminal and workspace to set up the project for you.",
      })}\n\n`);
      res.write("data: [DONE]\n\n");
      res.end();
      return;
    ...
    // ── Tool calling: Infinity AI can read his own source code (read-only) ──
    ...
    Promise.all([
      ...
      // Generate an auto-follow-up task when Infinity AI indicates one (contains "NEXT:")
      (() => {
    ...
    if (err instanceof LLMAllKeysCoolingError) {
      code = "llm_cooling";
      msg = "Infinity AI is recharging. All AI providers are cooling down. Try again in about 45 minutes.";
