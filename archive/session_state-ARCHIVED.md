...
- [x] Extract 7 presentational components into `artifacts/infinity/src/components/home/`:
...
- [x] routes/infinity/book.ts: POST /book/plan, POST /book/plan/review, GET /book/samples, POST /book/jobs, GET /book/jobs, GET /book/jobs/:id, POST /book/jobs/:id/cancel, GET /book/jobs/:id/pdf; BYO keys MASKED in every response
- [x] Registered bookRouter in routes/infinity/index.ts
...
- [x] studios-hub.tsx: 'book' studio card; app-overlays.tsx + home.tsx: 12s background polling of /api/infinity/book/jobs, push/browser/toast notification when a book completes, case 'book' in handleStudioSelect
...
- [x] Inspected repo: existing projects.ts schema (projects/projectChats/projectFiles/pins), routes/infinity/projects.ts CRUD, project-gallery.tsx, global userMemories + memories.ts routes, LLM extraction + injection in chat.ts, researchJobs, auto-migrate, i18n en/nl
