...
errors, SSE) and 64 in infinity string literals (labels, placeholders, toasts, aria). The AI
...
- Fix first: `chat.ts` personality prompts, voice-mode modifier, BUILD MODE block, capability
  ...
  `browse.ts` SSE/notification strings; `home.tsx`; `infinity-browser.tsx`; `app-overlays.tsx`;
...
- Move accent application to app-load scope (global hook or app root): read
  `localStorage['infinity-accent']` on startup, apply `--primary` and `--ring` on every load,
...
- Metadata: a SEPARATE Neon database (the user's explicit choice), Drizzle `files` table: id,
  ...
  storage_key, bucket, created_at, owner (user / infinity / account). Provide the schema
